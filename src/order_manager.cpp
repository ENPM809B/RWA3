...
// Using only arm_1 as of now
AriacOrderManager::AriacOrderManager(): arm1_{}
{
...
std::string AriacOrderManager::GetProductFrame(std::string product_type) {
    ...
    if (!product_frame_list_.empty()) {
        std::string frame = product_frame_list_[product_type].front();
...
void AriacOrderManager::ExecuteOrder() {
    ...
    // If the list is empty, no point proceeding further to process the order. So loop until you get a product.
    while( camera_.get_beam_count() != 1 ){
    	ros::spinOnce();
    	ros::Duration(0.5).sleep();
    }
...
//                ROS_INFO_STREAM("Product pose : " << product_type_pose_.second);
                std::string product_frame =  this->GetProductFrame(product.type);
                arm1_.customPartPick();
//                break;
