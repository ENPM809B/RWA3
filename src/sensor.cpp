 ...
 * Constructor
 */
AriacSensorManager::AriacSensorManager(): camera_on_belt_part_list{} {
	//camera1_part_list{}, camera2_part_list{}, camera3_part_list{}{
...
//                                                &AriacSensorManager::LogicalCamera3Callback, this);
    camera_on_belt_subscriber_ = sensor_nh_.subscribe("/ariac/logical_camera_on_belt", 10,
    											&AriacSensorManager::LogicalCameraConveyorBeltCallback, this);

    break_beam_subscriber_ = sensor_nh_.subscribe("/ariac/break_beam_1_change", 10, &AriacSensorManager::break_beam_callback, this);

//    camera1_frame_counter_ = 1;
//    camera2_frame_counter_ = 1;
//    camera3_frame_counter_ = 1;
    camera_on_belt_frame_counter_ = 0;
...
void AriacSensorManager::LogicalCameraConveyorBeltCallback(const osrf_gear::LogicalCameraImage::ConstPtr& image_msg){
//    if (init_) return;
    if (image_msg->models.size() == 0){
        //ROS_WARN_STREAM("Logical Camera on belt does not see anything");
    }
    else{
    	ROS_INFO_STREAM_THROTTLE(10,
    	                             "Logical camera on belt sees: '" << image_msg->models.size() << "' objects.");
    	current_parts_on_belt_ = *image_msg;
    	this->BuildProductFrames(4); // setting camera id as 4 for the camera placed on the conveyor belt
    }
}
...
//    }
    if (camera_id == 4) {
		for (auto& msg : current_parts_on_belt_.models) {
			//--build the frame for each product
			std::string product_frame = "logical_camera_on_belt_" + msg.type + "_" +
										std::to_string(camera_on_belt_frame_counter_) + "_frame";
			ROS_INFO_STREAM_THROTTLE(10,
			                             "product frame : '" << product_frame << "'.");
			product_frame_list_[msg.type].emplace_back(product_frame);
			camera_on_belt_frame_counter_++;
		}
		cam_on_belt_ = true;
	}
...
geometry_msgs::Pose AriacSensorManager::GetPartPose(const std::string& src_frame, const std::string& target_frame) {
    ...
    if (init_) {
//    	ROS_INFO_STREAM("Source frame is " << src_frame);
