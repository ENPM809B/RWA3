...
//    void LogicalCamera3Callback(const osrf_gear::LogicalCameraImage::ConstPtr &);
    void LogicalCameraConveyorBeltCallback(const osrf_gear::LogicalCameraImage::ConstPtr &);
    void break_beam_callback(const osrf_gear::Proximity::ConstPtr &);

    /*
     * Member functions
     */
    geometry_msgs::Pose GetPartPose(const std::string& src_frame, const std::string& target_frame);
...
//    ros::Subscriber camera_3_subscriber_;
    ros::Subscriber camera_on_belt_subscriber_, break_beam_subscriber_;
...
//    osrf_gear::LogicalCameraImage current_parts_3_;
    osrf_gear::LogicalCameraImage current_parts_on_belt_;
...
//    std::vector<AriacPartManager> camera1_part_list,camera2_part_list,camera3_part_list;
    std::vector<AriacPartManager> camera_on_belt_part_list;
